...
// ==========================================
const SUPABASE_URL = 'https://fbetpczzpwbutmgxmqqy.supabase.co'; 
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZiZXRwY3p6cHdidXRtZ3htcXF5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAxMjc4OTgsImV4cCI6MjA5NTcwMzg5OH0.4X5va5WwRTPN-HkWD4uHWBzR680NrTTNKCy4PwjNKak';
const ADMIN_PASSWORD = 'garisdarah'; 
const DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/1517900815237513247/8yfdFA0NmDNkG5PUISJvA3YKFKUbuwMIDDmuCvJrGA7BLMMaltC3wSf3buwYYc3SUUt7';
...
function showToast(title, icon='success') { Toast.fire({ icon: icon, title: title, background: isHackerMode ? '#000' : '#18181b', color: isHackerMode ? '#0f0' : '#f4f4f5' }); }
function showMessage(title, text, icon) { Swal.fire({ title: title, text: text, icon: icon, background: isHackerMode ? '#000' : '#18181b', color: isHackerMode ? '#0f0' : '#f4f4f5', confirmButtonColor: isHackerMode ? '#004400' : '#ea580c' }); }

function toggleHackerMode() {
    isHackerMode = !isHackerMode;
    if(isHackerMode) { document.body.classList.add('hacker-mode'); showToast('SYSTEM OVERRIDE: TERMINAL MODE ENGAGED', 'warning'); } 
    else { document.body.classList.remove('hacker-mode'); showToast('Terminal Mode Disengaged', 'info'); }
...
function confirmTransactionBox(title, text) {
    return Swal.fire({ title: title, text: text, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ea580c', cancelButtonColor: '#3f3f46', background: isHackerMode ? '#000' : '#18181b', color: isHackerMode ? '#0f0' : '#f4f4f5' }).then(result => result.isConfirmed);
}
...
async function handleLogout() {
    if(await confirmTransactionBox('Keluar?', "Sesi Anda akan diakhiri.")) { document.getElementById('login-password').value = ''; document.getElementById('app-container').classList.add('hidden'); document.getElementById('login-screen').classList.remove('hidden'); }
}
function showIDCard() {
    if(currentRole === 'tamu') return showToast('Tamu tidak memiliki ID Card', 'info');
    ...
    Swal.fire({
        html: `<div class="text-center p-2"><i class="fa-solid fa-id-card text-6xl text-orange-500 mb-4 drop-shadow-[0_0_15px_rgba(234,88,12,0.8)]"></i><h2 class="text-2xl font-bold font-mono text-zinc-100 uppercase tracking-widest">${currentOperatorName}</h2><p class="text-sm text-zinc-400 mb-6 font-bold tracking-widest">${currentRole.replace('_', ' ').toUpperCase()}</p><div class="bg-zinc-900/80 p-4 rounded-xl border border-zinc-700 text-left space-y-3 shadow-inner"><div class="flex justify-between items-center border-b border-zinc-800 pb-2"><span class="text-xs text-zinc-500 uppercase font-bold"><i class="fa-solid fa-fingerprint mr-2"></i>Status Akses</span> <span class="text-xs font-bold text-emerald-500 bg-emerald-900/30 px-2 py-0.5 rounded border border-emerald-800">GRANTED</span></div><div class="flex justify-between items-center border-b border-zinc-800 pb-2"><span class="text-xs text-zinc-500 uppercase font-bold"><i class="fa-solid fa-clipboard-check mr-2"></i>Aktivitas Pribadi</span> <span class="text-sm font-bold text-orange-400 font-mono">${totalActions} OPS</span></div><div class="flex justify-between items-center"><span class="text-xs text-zinc-500 uppercase font-bold"><i class="fa-solid fa-sack-dollar mr-2"></i>Sirkulasi Dana</span> <span class="text-sm font-bold text-emerald-400 font-mono">${formatMoney(totalVol)}</span></div></div></div>`,
        background: isHackerMode ? '#000' : '#18181b', color: isHackerMode ? '#0f0' : '#f4f4f5', showConfirmButton: false, showCloseButton: true
    });
}
...
function initLeafletMap() {
    ...
    const iconBoss = L.divIcon({ className: 'custom-div-icon', html: "<div class='w-8 h-8 bg-zinc-900 border-2 border-red-500 rounded-full flex items-center justify-center text-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]'><i class='fa-solid fa-motorcycle'></i></div>", iconSize: [32, 32], iconAnchor: [16, 16], popupAnchor: [0, -16] });
...
function updateMapPopups() {
    ...
    locations.forEach(loc => {
        ...
        const popupHTML = `<div class="min-w-[160px]"><h4 class="font-bold text-orange-500 border-b border-zinc-700 pb-1 mb-2 uppercase text-[11px] tracking-wider"><i class="fa-solid fa-location-crosshairs mr-1"></i> ${loc}</h4><div class="text-[11px] space-y-1 font-semibold"><div class="flex justify-between text-zinc-300"><span>Senjata (Tipe):</span> <span class="text-rose-400 font-mono">${countSenjata}</span></div><div class="flex justify-between text-zinc-300"><span>Resource (Tipe):</span> <span class="text-blue-400 font-mono">${countResource}</span></div><div class="flex justify-between border-t border-zinc-700 pt-1 mt-1 text-zinc-200"><span>Total Unit Item:</span> <span class="text-emerald-400 font-mono text-sm">${totalStok}</span></div></div></div>`;
        if (mapMarkers[loc]) { mapMarkers[loc].bindPopup(popupHTML); }
    }); showToast('Radar Disinkronisasi', 'info');
}
...
async function sendToDiscord(title, description, color, fields) {
    ...
    const payload = { username: "BLMC - WAREHOUSE", avatar_url: "https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/1f480.png", embeds: [{ title: title, description: description, color: color, fields: fields, timestamp: new Date().toISOString(), footer: { text: "Blmc Warehouse System" } }] };
...
async function adjustDirtyFunds() {
    if (currentRole === 'tamu') return; const { value: input } = await Swal.fire({ title: 'Penyesuaian UANG KOTOR', input: 'text', inputLabel: `Saldo Kotor: ${formatMoney(currentDirtyFunds)}\n(Gunakan minus (-) untuk menarik)`, showCancelButton: true, confirmButtonColor: '#ef4444', background: isHackerMode ? '#000' : '#18181b', color: isHackerMode ? '#0f0' : '#f4f4f5' }); if (!input) return; const amount = parseFloat(input); if (isNaN(amount)) return showMessage("Gagal", "Nominal tidak valid!", "error");
    const newBalance = currentDirtyFunds + amount; const { error } = await db.from('faction_funds').update({ balance: newBalance }).eq('id', 2); if (!error) { const type = amount >= 0 ? 'Deposit Kotor' : 'Tarik Kotor'; await db.from('vault_logs').insert([{ type: type, amount: Math.abs(amount), operator: getOperatorName() + " (Manual)" }]); await sendToDiscord(`💸 Log Brankas: ${type}`, `Operator **${getOperatorName()}** menyesuaikan Kotor.`, amount >= 0 ? 15105570 : 10038562, [{ name: "Nominal", value: formatMoney(Math.abs(amount)), inline: true }, { name: "Total Kotor", value: formatMoney(newBalance), inline: true }]); showToast('Uang Kotor disesuaikan', 'success'); await fetchFunds(); fetchTickerLogs(); }
}
async function fetchFundLogs() {
    const tbody = document.getElementById('fund-logs-table-body'); if(!tbody) return;
    try { const { data, error } = await db.from('vault_logs').select('*').order('created_at', { ascending: false }); if (error) throw error; tbody.innerHTML = data.length === 0 ? `<tr><td colspan="4" class="p-8 text-center text-zinc-500 italic">Belum ada riwayat mutasi kas.</td></tr>` : ''; data.forEach((log, index) => { const localTime = new Date(log.created_at).toLocaleString('id-ID', { hour12: false }); let typeBadge, amtColor, amtSign; if (log.type === 'Deposit') { typeBadge = `<span class="bg-emerald-950/80 text-emerald-400 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Setor</span>`; amtColor = 'text-emerald-400'; amtSign = '+'; } else if (log.type === 'Tarik') { typeBadge = `<span class="bg-rose-950/80 text-rose-400 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Tarik</span>`; amtColor = 'text-rose-400'; amtSign = '-'; } else if (log.type === 'Deposit Kotor') { typeBadge = `<span class="bg-red-950/80 text-red-500 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Setor Kotor</span>`; amtColor = 'text-red-500'; amtSign = '+'; } else { typeBadge = `<span class="bg-orange-950/80 text-orange-500 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Tarik Kotor</span>`; amtColor = 'text-orange-500'; amtSign = '-'; } tbody.innerHTML += `<tr class="${index % 2 === 0 ? 'bg-zinc-950/50' : 'bg-transparent'} hover:bg-zinc-800/20"><td class="p-3 text-center font-mono text-zinc-500">${localTime}</td><td class="p-3 text-center">${typeBadge}</td><td class="p-3 text-right font-mono font-bold ${amtColor}">${amtSign}${formatMoney(log.amount)}</td><td class="p-3 text-zinc-400 text-xs font-semibold flex items-center gap-2"><i class="fa-solid fa-user-tie text-zinc-600"></i> ${log.operator}</td></tr>`; }); } catch (err) {}
}
...
async function processTransaction(updates, totalQty, itemName, location, type, unitPrice, totalPrice, target, isPaket = false) {
    for (let u of updates) { await db.from('inventory').update({ stock: u.newStock }).eq('id', u.id); } const newFunds = type === 'Jual' ? currentFunds + totalPrice : currentFunds - totalPrice; await db.from('faction_funds').update({ balance: newFunds }).eq('id', 1); await db.from('warehouse_logs').insert([{ item_name: itemName, location: location, quantity: totalQty, type: type, operator: getOperatorName(), price: unitPrice, total_price: totalPrice, target_name: target }]); const vType = type === 'Jual' ? 'Deposit' : 'Tarik'; await db.from('vault_logs').insert([{ type: vType, amount: totalPrice, operator: `${getOperatorName()} | Bisnis ${isPaket?'Paket':'Satuan'}` }]); await sendToDiscord(`💸 Transaksi Bisnis: ${type}`, `Operator **${getOperatorName()}** mengeksekusi bisnis.`, type==='Jual'?3066993:15158332, [{name: "Item", value: itemName, inline: true}, {name: "Jumlah", value: `${totalQty}`, inline: true}, {name: "Total Kas", value: formatMoney(totalPrice), inline: true}]); showToast(`Transaksi Berhasil!`, 'success'); await fetchInventory(); fetchTickerLogs(); switchTab('dashboard');
}
async function handleBusinessSatuan(e) {
    e.preventDefault(); if(currentRole === 'tamu') return; const type = document.getElementById('satuan-tipe').value; const itemId = parseInt(document.getElementById('satuan-item').value); if(!itemId) return showMessage('Peringatan', 'Pilih barang!', 'warning'); const qty = parseInt(document.getElementById('satuan-qty').value); const price = parseFloat(document.getElementById('satuan-harga').value); const target = document.getElementById('satuan-target').value; const item = inventoryData.find(i => i.id === itemId); if (!item) return; const totalPrice = qty * price;
    if (type === 'Beli') { if (currentFunds < totalPrice) { if(!await confirmTransactionBox('Saldo Kurang!', `Tetap paksa beli?`)) return; } await processTransaction([{id: item.id, newStock: item.stock + qty}], qty, item.name, item.location, 'Beli', price, totalPrice, target); } else { if (item.stock < qty) return showMessage('Stok Kurang', `Sisa stok: ${item.stock}.`, 'error'); await processTransaction([{id: item.id, newStock: item.stock - qty}], qty, item.name, item.location, 'Jual', price, totalPrice, target); } document.getElementById('form-bisnis-satuan').reset(); document.getElementById('display-satuan-total').innerText = '$ 0';
}
async function handleBusinessPaket(e) {
    e.preventDefault(); if(currentRole === 'tamu') return; const type = document.getElementById('paket-tipe').value; const bId = document.getElementById('paket-bundle').value; if(!bId) return showMessage('Peringatan', 'Pilih paket!', 'warning'); const qty = parseInt(document.getElementById('paket-qty').value); const target = document.getElementById('paket-target').value; const bundle = bundlesData.find(x => x.id == bId); if(!bundle) return; const totalPrice = qty * bundle.price; let updates = []; let itemsToCheck = [ {id: bundle.item1_id, q: bundle.item1_qty}, {id: bundle.item2_id, q: bundle.item2_qty}, {id: bundle.item3_id, q: bundle.item3_qty} ].filter(x => x.id);
    for (let req of itemsToCheck) { const item = inventoryData.find(x => x.id == req.id); if(!item) return showMessage('Error', 'Barang di paket terhapus dari gudang!', 'error'); const needed = req.q * qty; if(type === 'Jual' && item.stock < needed) return showMessage('Stok Kurang', `Stok ${item.name} kurang! Butuh ${needed}, Sisa ${item.stock}`, 'error'); updates.push({id: item.id, newStock: type === 'Jual' ? item.stock - needed : item.stock + needed}); }
    if (type === 'Beli' && currentFunds < totalPrice) { if(!await confirmTransactionBox('Saldo Kurang!', `Tetap paksa beli?`)) return; } await processTransaction(updates, qty, `[PAKET] ${bundle.name}`, 'Bundle Area', type, bundle.price, totalPrice, target, true); document.getElementById('form-bisnis-paket').reset(); document.getElementById('display-paket-total').innerText = '$ 0';
}
async function handleCreateBundle(e) {
    e.preventDefault(); if(currentRole === 'tamu') return; const name = document.getElementById('create-bundle-name').value; const price = document.getElementById('create-bundle-price').value; const i1 = document.getElementById('bundle-item1').value; const q1 = document.getElementById('bundle-qty1').value; const i2 = document.getElementById('bundle-item2').value; const q2 = document.getElementById('bundle-qty2').value; const i3 = document.getElementById('bundle-item3').value; const q3 = document.getElementById('bundle-qty3').value; if(!i1) return showMessage('Gagal', 'Paket butuh minimal 1 barang.', 'error');
    const {error} = await db.from('bundles').insert([{ name: name, price: price, item1_id: i1||null, item1_qty: q1||null, item2_id: i2||null, item2_qty: q2||null, item3_id: i3||null, item3_qty: q3||null }]); if(!error) { showToast('Paket Dibuat', 'success'); document.getElementById('form-buat-paket').reset(); fetchBundles(); } else showMessage('Error', error.message, 'error');
}
async function deleteBundle(id) { if(currentRole === 'tamu') return; if(await confirmTransactionBox('Hapus Paket?', 'Paket akan dihapus dari daftar.')) { const {error} = await db.from('bundles').delete().eq('id', id); if(!error) { showToast('Terhapus', 'success'); fetchBundles(); } } }

// ==========================================
// 📊 CHART & LOGS
// ==========================================
function renderAnalyticsChart() {
    const canvas = document.getElementById('bloodlineChart'); if (!canvas) return; const ctx = canvas.getContext('2d'); if (liveChartInstance) liveChartInstance.destroy(); liveChartInstance = new Chart(ctx, { type: 'bar', data: { labels: inventoryData.map(i => i.name), datasets: [{ data: inventoryData.map(i => i.stock), backgroundColor: inventoryData.map(i => i.stock <= currentCriticalLimit ? 'rgba(239, 68, 68, 0.6)' : (isHackerMode ? 'rgba(0,255,0,0.4)' : 'rgba(234, 88, 12, 0.5)')), borderColor: inventoryData.map(i => i.stock <= currentCriticalLimit ? 'rgba(239, 68, 68, 1)' : (isHackerMode ? '#0f0' : 'rgba(234, 88, 12, 1)')), borderWidth: 2, borderRadius: 5 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: isHackerMode ? '#0f0' : '#a1a1aa' }, grid: { color: isHackerMode ? '#004400' : '#27272a' } }, y: { ticks: { color: isHackerMode ? '#0f0' : '#a1a1aa' }, grid: { color: isHackerMode ? '#004400' : '#27272a' } } } } });
}
...
async function fetchTickerLogs() {
    const ticker = document.getElementById('live-ticker'); if (!ticker) return; try { const { data, error } = await db.from('warehouse_logs').select('*').order('created_at', { ascending: false }).limit(5); if (error || !data || data.length === 0) { ticker.innerText = "Sistem Gudang BLMC Beroperasi Normal."; return; } ticker.innerText = data.map(l => `[${new Date(l.created_at).toLocaleTimeString('id-ID')}] ${l.operator}: ${l.type} ${l.quantity}x ${l.item_name}`).join('  ///  '); } catch (e) { ticker.innerText = "BLMC Stockade Network Active."; }
}
...
async function exportMonthlyRecapExcel() {
    const tM = document.getElementById('rekap-bulan').value; const tY = document.getElementById('rekap-tahun').value; try { const { data: logs, error } = await db.from('warehouse_logs').select('*').order('created_at', { ascending: true }); if (error) throw error; const fLogs = logs.filter(l => { const d = new Date(l.created_at); return String(d.getMonth() + 1).padStart(2, '0') === tM && String(d.getFullYear()) === tY; }); if (fLogs.length === 0) return showMessage('Info', `Tidak ada transaksi di ${tM}-${tY}.`, 'info'); const rows = fLogs.map((l, i) => ({ "No": i + 1, "Waktu": new Date(l.created_at).toLocaleString('id-ID'), "Item": l.item_name, "Jenis": l.price > 0 ? (l.type==='Beli'||l.type==='Masuk'?'Beli':'Jual') : (l.type==='Simpan'||l.type==='Masuk'?'Deposit':'Ambil'), "Qty": l.quantity, "Ket": l.target_name || '-', "Harga": l.price || 0, "Total Kas": l.total_price || 0, "Operator": l.operator })); const ws = XLSX.utils.json_to_sheet(rows); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Ledger"); XLSX.writeFile(wb, `AUDIT_BLMC_${tM}_${tY}.xlsx`); showToast('Laporan diunduh!', 'success'); } catch (err) { showMessage('Error', err.message, 'error'); }
}
async function handleMutasiGudang(e) {
    e.preventDefault(); if(currentRole === 'tamu') return; const type = document.getElementById('mutasi-tipe').value; const itemId = parseInt(document.getElementById('mutasi-item').value); if(!itemId) return showMessage('Peringatan', 'Pilih barang!', 'warning'); const qty = parseInt(document.getElementById('mutasi-qty').value); const target = document.getElementById('mutasi-keterangan').value; const item = inventoryData.find(i => i.id === itemId); if (!item) return; let newStock = item.stock; if(type === 'Ambil') { if (item.stock < qty) return showMessage('Stok Kurang', `Sisa: ${item.stock}.`, 'error'); newStock = item.stock - qty; } else { newStock = item.stock + qty; } const { error } = await db.from('inventory').update({ stock: newStock }).eq('id', item.id); if (!error) { await db.from('warehouse_logs').insert([{ item_name: item.name, location: item.location, quantity: qty, type: type, operator: getOperatorName(), price: 0, total_price: 0, target_name: target }]); await sendToDiscord(`🔫 Mutasi Logistik: ${type}`, `Operator **${getOperatorName()}** memindahkan barang.`, type==='Ambil'?10181046:3447003, [{ name: "Barang", value: item.name, inline: true }, { name: "Jumlah", value: `${qty}`, inline: true }, { name: "Alasan", value: target, inline: true }]); document.getElementById('form-mutasi-gudang').reset(); showToast(`Mutasi tersimpan!`, 'success'); await fetchInventory(); fetchTickerLogs(); switchTab('dashboard'); }
}
async function handleAddNewItem(e) {
    e.preventDefault(); if(currentRole === 'tamu') return; const name = document.getElementById('input-tambah-nama').value; const loc = document.getElementById('input-tambah-lokasi').value; const stock = parseInt(document.getElementById('input-tambah-stok').value) || 0; const cat = document.getElementById('input-tambah-kategori').value; const { error } = await db.from('inventory').insert([{ name: name, location: loc, stock: stock, category: cat }]); if (!error) { await db.from('warehouse_logs').insert([{ item_name: name, location: loc, quantity: stock, type: 'Simpan', operator: getOperatorName(), price: 0, total_price: 0, target_name: 'Registrasi Baru' }]); await sendToDiscord(`📦 Item Baru Terdaftar`, `Operator mencatat barang.`, 3447003, [{ name: "Nama", value: name, inline: true }]); showToast('Terdaftar!', 'success'); document.getElementById('form-tambah-barang').reset(); await fetchInventory(); fetchTickerLogs(); switchTab('dashboard'); } else showMessage('Gagal', error.message, 'error');
}
async function handleEditItem(id) {
    if(currentRole === 'tamu') return; const item = inventoryData.find(i => i.id === id); if(!item) return; const { value: v } = await Swal.fire({ title: 'Edit Barang', html: `<div class="space-y-4 my-2"><input id="swal-edit-name" class="swal2-input !m-0 !bg-zinc-900 !text-zinc-200" value="${item.name}"><select id="swal-edit-category" class="swal2-select !m-0 !bg-zinc-900 !text-zinc-200"><option value="Resource" ${item.category === 'Resource' ? 'selected' : ''}>Resource</option><option value="Senjata" ${item.category === 'Senjata' ? 'selected' : ''}>Senjata</option></select><select id="swal-edit-location" class="swal2-select !m-0 !bg-zinc-900 !text-zinc-200"><option value="Brankas Bos" ${item.location === 'Brankas Bos' ? 'selected' : ''}>Brankas Bos</option><option value="Loker Bersama" ${item.location === 'Loker Bersama' ? 'selected' : ''}>Loker Bersama</option><option value="Gudang Umum" ${item.location === 'Gudang Umum' ? 'selected' : ''}>Gudang Umum</option></select></div>`, showCancelButton: true, confirmButtonColor: '#3b82f6', background: isHackerMode ? '#000' : '#18181b', color: isHackerMode ? '#0f0' : '#f4f4f5', preConfirm: () => { const n = document.getElementById('swal-edit-name').value; if(!n) { Swal.showValidationMessage('Nama kosong!'); return false; } return { name: n, category: document.getElementById('swal-edit-category').value, location: document.getElementById('swal-edit-location').value } } }); if (v) { const { error } = await db.from('inventory').update({ name: v.name, category: v.category, location: v.location }).eq('id', id); if (!error) { showToast('Di-update!', 'success'); await fetchInventory(); } else showMessage('Error', error.message, 'error'); }
}
async function handleDeleteItem(id) {
    if(currentRole === 'tamu') return; if(await confirmTransactionBox('Hapus Data?', 'Awas! Jika barang ini ada di dalam Paket, sistem Paket akan error!')) { const { error } = await db.from('inventory').delete().eq('id', id); if (!error) { showToast('Terhapus', 'info'); await fetchInventory(); } }
}

// ==========================================
// 💀 MANAJEMEN ROSTER & ANGGOTA
// ==========================================
...
function renderMembersTable() {
    ...
    membersData.forEach((member, index) => { let statusBadge = ''; if(member.status === 'Aktif') statusBadge = '<span class="bg-emerald-950/80 text-emerald-400 border border-emerald-800 px-2 py-0.5 rounded text-[10px] font-bold uppercase">🟢 Aktif</span>'; else if(member.status === 'Penjara') statusBadge = '<span class="bg-rose-950/80 text-rose-400 border border-rose-800 px-2 py-0.5 rounded text-[10px] font-bold uppercase">🔴 Penjara</span>'; else statusBadge = '<span class="bg-zinc-800 text-zinc-400 border border-zinc-700 px-2 py-0.5 rounded text-[10px] font-bold uppercase">⚪ Inaktif</span>'; let rankStyle = 'text-zinc-200'; if(member.rank.includes('President') || member.rank.includes('Sgt') || member.rank.includes('Treasury')) rankStyle = 'text-orange-400 font-bold'; else if(member.rank === 'Prospect' || member.rank === 'Hangaround') rankStyle = 'text-zinc-500'; const actionBtn = isStaff ? `<button onclick="handleDeleteMember(${member.id}, '${member.name}')" class="text-red-500 hover:text-white hover:bg-red-600 border border-red-900/50 px-3 py-1 rounded transition-all mr-1"><i class="fa-solid fa-user-xmark"></i></button>` : ''; tbody.innerHTML += `<tr class="${index % 2 === 0 ? 'bg-zinc-950/50' : 'bg-transparent'} hover:bg-zinc-900/50 transition-colors"><td class="p-3 pl-5 font-bold tracking-wide text-zinc-100">${member.name}</td><td class="p-3 text-sm ${rankStyle}">${member.rank}</td><td class="p-3 text-center">${statusBadge}</td><td class="p-3 text-center admin-only ${isStaff ? '' : 'hidden'}">${actionBtn}</td></tr>`; });
}
async function handleAddMember(e) { e.preventDefault(); if(currentRole === 'tamu') return; const name = document.getElementById('roster-name').value; const rank = document.getElementById('roster-rank').value; const status = document.getElementById('roster-status').value; const { error } = await db.from('members').insert([{ name: name, rank: rank, status: status }]); if (!error) { showToast('Anggota Terdaftar!', 'success'); document.getElementById('form-tambah-anggota').reset(); fetchMembers(); } else showMessage('Gagal', error.message, 'error'); }
async function handleDeleteMember(id, name) { if(currentRole === 'tamu') return; if(await confirmTransactionBox('Pecat / Hapus Data?', `Hapus ${name} dari BLMC?`)) { const { error } = await db.from('members').delete().eq('id', id); if(!error) { showToast('Dihapus', 'info'); fetchMembers(); } } }

// ==========================================
// 📜 SOP VAULT (ATURAN FAKSI)
// ==========================================
...
async function handleAddRule(e) {
    ...
    if(!error) { showToast('Aturan Diterbitkan!', 'success'); document.getElementById('form-tambah-rule').reset(); await sendToDiscord(`📜 SOP Baru Diterbitkan`, `**${title}**\n*Oleh: ${getOperatorName()}*`, 15158332, []); fetchRules(); } else showMessage('Gagal Menyimpan', error.message, 'error');
}

async function handleDeleteRule(id) {
    if(currentRole === 'tamu') return;
    if(await confirmTransactionBox('Cabut Aturan?', 'Yakin ingin mencabut pasal ini dari buku panduan faksi?')) { const {error} = await db.from('faction_rules').delete().eq('id', id); if(!error) { showToast('Pasal Dicabut', 'info'); fetchRules(); } }
}
